...
─────
  ...
  List countries   : python sector_comtrade_pipeline.py --config textiles_sector_config.xlsx --mode list_countries
  Geography drill-down (bilateral reporter<->partner lookup, e.g. China imports from the US):
                     python sector_comtrade_pipeline.py --config textiles_sector_config.xlsx --mode bilateral \
                            --reporter China --partner "United States" --flow import
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# GEOGRAPHY DRILL-DOWN MODE — BILATERAL REPORTER ↔ PARTNER LOOKUP
# On-demand reporter+partner trade lookup (e.g. "how much does China import
# from the US") — every other mode above only ever fetches Reporter→World
# totals (Partner fixed to "World"). This section adds a true bilateral
# fetch, a country-name/ISO3/code reference + resolver, a local result
# cache, and a Geography_Bilateral sheet writer. Nothing here is ever
# pre-computed for every country pair — only for the one pair a user picks.
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
_COUNTRY_REF_CACHE_FILE       = "comtrade_country_reference.json"
_COUNTRY_REF_TTL_DAYS         = 30   # country list rarely changes; refetch monthly
_BILATERAL_MAX_HS_PER_CALL    = 20   # Comtrade API limit on comma-joined cmdCode
_BILATERAL_MAX_YEARS_PER_CALL = 12   # Comtrade API limit on comma-joined period


def get_country_reference(force_refresh=False):
    """Return a DataFrame of [code, name, iso3, is_group] for every country/
    area Comtrade recognises. Used to populate the Geography Drill-Down
    reporter/partner dropdowns in app.py and to resolve typed names/ISO3/
    codes on the CLI. The same code space is used for both reporter and
    partner roles, so one reference list serves both dropdowns.

    Cached locally (comtrade_country_reference.json) since this list barely
    changes and the source endpoint is public (no API key needed) — the
    cache just avoids a network round-trip on every Streamlit rerun."""
    cache_path = Path(_COUNTRY_REF_CACHE_FILE)
    if not force_refresh and cache_path.exists():
        age_days = (time.time() - cache_path.stat().st_mtime) / 86400
        if age_days < _COUNTRY_REF_TTL_DAYS:
            try:
                return pd.read_json(cache_path, dtype={"code": str})
            except Exception:
                pass  # fall through and refetch

    import comtradeapicall
    try:
        raw = comtradeapicall.getReference("reporter")
        if raw is None or raw.empty:
            raise RuntimeError("Empty reference data returned")
        raw.columns = [str(c).strip() for c in raw.columns]
        name_col = "reporterDesc" if "reporterDesc" in raw.columns else "text"
        ref = pd.DataFrame({
            "code":     raw["reporterCode"].astype(str),
            "name":     raw[name_col].astype(str),
            "iso3":     raw.get("reporterCodeIsoAlpha3", pd.Series([""] * len(raw))).astype(str),
            "is_group": raw.get("isGroup", pd.Series([False] * len(raw))).astype(bool),
        }).drop_duplicates(subset=["code"]).sort_values("name").reset_index(drop=True)
        ref.to_json(cache_path, orient="records")
        return ref
    except Exception as e:
        if cache_path.exists():
            log.warning(f"  ⚠  Could not refresh country reference list ({e}); using stale local cache.")
            return pd.read_json(cache_path, dtype={"code": str})
        raise RuntimeError(
            f"Could not fetch the Comtrade country reference list and no local cache "
            f"exists yet ({e}). An internet connection is required the first time "
            f"Geography Drill-Down is used."
        )


def resolve_country_code(value, ref_df=None):
    """Resolve a country typed/selected as a name, ISO3 code, or numeric
    Comtrade code into (code, name, iso3). Raises ValueError with close-match
    suggestions if nothing matches."""
    ref_df = ref_df if ref_df is not None else get_country_reference()
    raw = str(value).strip()
    if not raw:
        raise ValueError("No country given.")

    if raw.isdigit():
        hit = ref_df[ref_df["code"] == raw]
        if not hit.empty:
            r = hit.iloc[0]
            return str(r["code"]), r["name"], r["iso3"]

    if len(raw) == 3 and raw.isalpha():
        hit = ref_df[ref_df["iso3"].str.upper() == raw.upper()]
        if not hit.empty:
            r = hit.iloc[0]
            return str(r["code"]), r["name"], r["iso3"]

    hit = ref_df[ref_df["name"].str.lower() == raw.lower()]
    if hit.empty:
        hit = ref_df[ref_df["name"].str.lower().str.contains(raw.lower(), na=False, regex=False)]
    if not hit.empty:
        r = hit.iloc[0]
        return str(r["code"]), r["name"], r["iso3"]

    close = ref_df[ref_df["name"].str.lower().str.startswith(raw.lower()[:3])]["name"].head(5).tolist()
    suggestion = f" Did you mean: {', '.join(close)}?" if close else ""
    raise ValueError(f"Could not resolve country '{value}' to a Comtrade code.{suggestion}")
...
def _bilateral_cache_file(config):
    return f"{config['slug']}_bilateral_cache.json"


def _load_bilateral_cache(config):
    p = Path(_bilateral_cache_file(config))
    if p.exists():
        try:
...
def _save_bilateral_cache(config, cache):
    try:
        Path(_bilateral_cache_file(config)).write_text(json.dumps(cache), encoding="utf-8")
    except Exception as e:
        log.warning(f"  ⚠  Could not save bilateral cache: {e}")


def fetch_bilateral(config, reporter_code, partner_code, flow_code, hs_codes, years,
                     mode_label="bilateral", use_cache=True):
    """On-demand reporter↔partner trade fetch via comtradeapicall.getBilateralData.
    One API call per (HS-code batch × year batch) — batches exist only to
    respect the Comtrade API's hard per-call limits (20 HS codes / 12 years),
    never to build a country×country matrix: reporter and partner are always
    exactly the one pair passed in. Already-fetched (reporter, partner, flow,
    HS-batch, year-batch) combinations are served from a local on-disk cache
    instead of re-calling the API."""
    import comtradeapicall

    api_key  = os.environ.get(config["api_key_var"], "")
    hs_codes = [str(c).strip().zfill(2) for c in hs_codes]
    years    = [str(y).strip() for y in years]
    cache    = _load_bilateral_cache(config) if use_cache else {}

    frames, hit_cache = [], 0
    hs_chunks = _chunk(hs_codes, _BILATERAL_MAX_HS_PER_CALL)
    yr_chunks = _chunk(years, _BILATERAL_MAX_YEARS_PER_CALL)

    for hs_chunk in hs_chunks:
        for yr_chunk in yr_chunks:
            key = "|".join([str(reporter_code), str(partner_code), str(flow_code),
                            ",".join(sorted(hs_chunk)), ",".join(sorted(yr_chunk))])
            ...
            period_str = ",".join(yr_chunk)
            cmd_str    = ",".join(hs_chunk)
            log.info(f"  → Bilateral fetch: reporter={reporter_code} partner={partner_code} "
                     f"flow={flow_code} HS={cmd_str} years={period_str}")
            try:
                mydf = comtradeapicall.getBilateralData(
                    api_key, typeCode="C", freqCode="A", clCode="HS",
                    period=period_str, reporterCode=reporter_code, cmdCode=cmd_str,
                    flowCode=flow_code, partnerCode=partner_code,
                    maxRecords=None, format_output="JSON", includeDesc=True,
                )
                if mydf is None or mydf.empty:
                    log.warning(f"     ⚠  No bilateral data returned for HS {cmd_str}, years {period_str}")
                    cache[key] = []
                    record_error(mode_label, f"{flow_code}:{reporter_code}->{partner_code}",
                                 cmd_str, period_str, "NoData", "API returned empty/None",
                                 "Skipped — logged")
                else:
                    log.info(f"     ✓  {len(mydf)} rows")
                    frames.append(mydf)
                    cache[key] = mydf.to_dict(orient="records")
            except Exception:
                emsg = traceback.format_exc().strip().split("\n")[-1]
                log.error(f"     ✗  ERROR for HS {cmd_str}, years {period_str}: {emsg}")
                record_error(mode_label, f"{flow_code}:{reporter_code}->{partner_code}",
                             cmd_str, period_str, "APIError", emsg,
                             "Skipped — logged" if config["continue_on_error"] else "Halted")
                if not config["continue_on_error"]:
                    log.error("Continue-on-error is OFF. Stopping bilateral fetch.")
                    if use_cache:
                        _save_bilateral_cache(config, cache)
    ...
    if use_cache:
        _save_bilateral_cache(config, cache)
    if hit_cache:
        log.info(f"  ℹ  Served {hit_cache} batch(es) from local cache (no API call needed).")
    return pd.concat(frames, ignore_index=True, sort=False) if frames else pd.DataFrame()


def build_bilateral_rows(df_raw, reporter_name, reporter_iso3, partner_name, partner_iso3, config):
    """Convert raw getBilateralData() output into the Geography_Bilateral
    sheet schema. Reuses the exact Quantity/Unit Price convention from
    build_master_table: qty<=0 is treated as missing, not zero/inf, so
    Quantity/Unit Price stay blank rather than showing a bogus $0.00-per-
    unit or infinite price."""
    cols = ["Year", "Reporter", "Reporter ISO3", "Partner", "Partner ISO3", "Flow",
            "HS Code", "HS Description", "Trade Value (USD)", "Quantity", "Qty Unit", "Unit Price"]
    if df_raw is None or df_raw.empty:
        return pd.DataFrame(columns=cols)

    cmd_raw    = _get(df_raw, ["cmdCode", "cmdcode"]).astype(str).str.zfill(2)
    trade_usd  = pd.to_numeric(_get(df_raw, ["primaryValue", "primaryvalue"]), errors="coerce")
    qty_raw    = pd.to_numeric(_get(df_raw, ["qty"]), errors="coerce")
    qty_valid  = qty_raw.where(qty_raw > 0)
    qty_unit   = _get(df_raw, ["qtyUnitAbbr", "qtyunitabbr"]).astype(str).where(qty_valid.notna(), "")
    unit_price = (trade_usd / qty_valid).round(4)

    out = pd.DataFrame({
        "Year":              pd.to_numeric(_get(df_raw, ["period"]), errors="coerce"),
        "Reporter":          _get(df_raw, ["reporterDesc", "reporterdesc"], fallback=reporter_name),
        "Reporter ISO3":     reporter_iso3,
        "Partner":           _get(df_raw, ["partnerDesc", "partnerdesc"], fallback=partner_name),
        "Partner ISO3":      partner_iso3,
        "Flow":              _get(df_raw, ["flowDesc", "flowdesc"]),
        "HS Code":           cmd_raw,
        "HS Description":    _get(df_raw, ["cmdDesc", "cmddesc"]).astype(str),
...
def write_bilateral_sheet(config, new_rows):
    """Append new_rows to the Geography_Bilateral sheet in the cleaned
    workbook, de-duplicating on (Reporter, Partner, Flow, HS Code, Year) so
    re-running the same lookup never creates duplicate rows (last fetch
    wins). Creates the workbook if it doesn't exist yet — Geography
    Drill-Down can be used even before --mode clean has ever been run.
    Follows the exact same delete-then-append pattern as run_competitor()
    so other sheets in the workbook are never disturbed."""
    cleaned = config["cleaned_file"]
    sh      = "Geography_Bilateral"
    dedupe  = ["Reporter", "Partner", "Flow", "HS Code", "Year"]

    existing = pd.DataFrame(columns=new_rows.columns)
    if os.path.exists(cleaned):
        try:
            xls = pd.ExcelFile(cleaned)
            if sh in xls.sheet_names:
                existing = pd.read_excel(xls, sh, header=1, dtype={"HS Code": str})
                existing.columns = [str(c).strip() for c in existing.columns]
        except Exception as e:
            log.warning(f"  ⚠  Could not read existing {sh} sheet, starting fresh: {e}")
    ...
    if not combined.empty:
        ...
        sort_cols = [c for c in ["Year", "Reporter", "Partner", "Flow", "HS Code"] if c in combined.columns]
        if sort_cols:
    ...
    if os.path.exists(cleaned):
        ...
        if sh in wb.sheetnames:
            del wb[sh]
        if wb.sheetnames:
            # Normal case: other sheets remain (Master_Table, Competitor_*, etc.)
            wb.save(cleaned)
            wb.close()
            with pd.ExcelWriter(cleaned, engine="openpyxl", mode="a") as writer:
                ws = writer.book.create_sheet(sh)
                _fast_write_df(ws, combined, start_row=1, title_text="Geography Drill-Down — Bilateral Trade")
        else:
            # Edge case: Geography_Bilateral was the ONLY sheet in the
            # workbook, so deleting it left zero sheets. openpyxl refuses to
            # save a workbook with no visible sheets, so rebuild from scratch
            # instead of delete-then-append.
            wb.close()
            with pd.ExcelWriter(cleaned, engine="openpyxl", mode="w") as writer:
                ws = writer.book.create_sheet(sh)
                _fast_write_df(ws, combined, start_row=1, title_text="Geography Drill-Down — Bilateral Trade")
    else:
        with pd.ExcelWriter(cleaned, engine="openpyxl", mode="w") as writer:
    ...
    log.info(f"  ✅  Geography_Bilateral sheet updated → {cleaned}  "
             f"({len(combined)} total rows, {len(new_rows)} from this lookup)")
    return combined


def run_bilateral(config, reporter, partner, flow, hs_codes=None, years=None, is_test=False):
    """CLI/UI entry point for one Geography Drill-Down lookup. `reporter`/
    `partner` may be a country name, ISO3 code, or numeric Comtrade code.
    `flow` is 'X'/'export(s)' or 'M'/'import(s)'. hs_codes/years default to
    the active sector config's full HS list / year range when omitted."""
    mode_label = "bilateral"
    ref = get_country_reference()
    r_code, r_name, r_iso3 = resolve_country_code(reporter, ref)
    p_code, p_name, p_iso3 = resolve_country_code(partner, ref)
    flow_code = "X" if str(flow).strip().upper() in ("X", "EXPORT", "EXPORTS") else "M"

    use_hs = [str(h).strip() for h in hs_codes] if hs_codes else config["hs_codes"]
    use_years = [str(y).strip() for y in years] if years else [str(y) for y in config["years_list"]]
    if is_test:
        use_hs    = use_hs[:1]
        use_years = use_years[-1:]

    log.info("\n" + "█" * 60)
    log.info(f"  GEOGRAPHY DRILL-DOWN — {r_name} ({r_code}) → {p_name} ({p_code})")
    log.info(f"  Flow: {'Exports' if flow_code == 'X' else 'Imports'}  |  "
             f"HS codes: {len(use_hs)}  |  Years: {use_years[0]}–{use_years[-1]}")
    log.info("█" * 60)

    raw  = fetch_bilateral(config, r_code, p_code, flow_code, use_hs, use_years, mode_label=mode_label)
    rows = build_bilateral_rows(raw, r_name, r_iso3, p_name, p_iso3, config)

    if rows.empty:
        log.warning("  ⚠  No bilateral data returned for this pair/flow/years — nothing added to the workbook.")
        return rows

    write_bilateral_sheet(config, rows)
    log.info(f"\n  ✅  GEOGRAPHY DRILL-DOWN COMPLETE. {len(rows)} new row(s) added to "
             f"Geography_Bilateral in {config['cleaned_file']}.")
    return rows


def fetch_reporter_total(config, reporter_code, flow_code, hs_codes, years,
                          mode_label="bilateral_total", use_cache=True):
    """Reporter-vs-World total fetch (Partner fixed to World/code 0) — used
    only as the denominator for the Geography Drill-Down 'compare multiple
    partners' share view (% of the reporter's true total + a 'Rest of World'
    remainder), never to build a country×country matrix. Calls getFinalData
    (the same call the rest of this pipeline already uses for every
    Reporter→World fetch) rather than getBilateralData, since the latter is
    built for two *specific* countries, not totals. Mirrors fetch_bilateral's
    chunking/caching exactly and shares its on-disk cache file, under a
    distinct 'TOTAL|...' key prefix so the two never collide."""
    import comtradeapicall
    ...
    for hs_chunk in hs_chunks:
        for yr_chunk in yr_chunks:
            key = "|".join(["TOTAL", str(reporter_code), str(flow_code),
                            ",".join(sorted(hs_chunk)), ",".join(sorted(yr_chunk))])
            if use_cache and key in cache:
                rows = cache[key]
                if rows:
            ...
            period_str = ",".join(yr_chunk)
            cmd_str    = ",".join(hs_chunk)
            log.info(f"  → World-total fetch: reporter={reporter_code} flow={flow_code} "
                     f"HS={cmd_str} years={period_str}")
            try:
                mydf = comtradeapicall.getFinalData(
                    api_key, typeCode="C", freqCode="A", clCode="HS",
                    period=period_str, reporterCode=reporter_code, cmdCode=cmd_str,
                    flowCode=flow_code, partnerCode="0", partner2Code=None,
                    customsCode=None, motCode=None, maxRecords=None,
                    format_output="JSON", aggregateBy=None, breakdownMode="classic",
                    countOnly=None, includeDesc=True,
                )
                if mydf is None or mydf.empty:
                    log.warning(f"     ⚠  No world-total data returned for HS {cmd_str}, years {period_str}")
                    cache[key] = []
                    record_error(mode_label, f"{flow_code}:{reporter_code}->World",
                                 cmd_str, period_str, "NoData", "API returned empty/None",
                                 "Skipped — logged")
                else:
                    log.info(f"     ✓  {len(mydf)} rows")
                ...
                record_error(mode_label, f"{flow_code}:{reporter_code}->World",
                             ...
                             "Skipped — logged" if config["continue_on_error"] else "Halted")
                if not config["continue_on_error"]:
                    log.error("Continue-on-error is OFF. Stopping world-total fetch.")
                    if use_cache:
                        _save_bilateral_cache(config, cache)
                    return pd.concat(frames, ignore_index=True, sort=False) if frames else pd.DataFrame()
            time.sleep(0.5)
...
def run_bilateral_compare(config, reporter, partners, flow, hs_codes=None, years=None,
                          include_world=True, is_test=False):
    """CLI/UI entry point for the Geography Drill-Down 'compare multiple
    partners' view: one reporter, several partner countries, each fetched
    on-demand (one API call per partner — never a full country×country
    matrix), plus one optional World-total call so the resulting chart can
    show an accurate % share of the reporter's real total (with a 'Rest of
    World' remainder) instead of just a share among the countries picked.
    Every row lands in the same Geography_Bilateral sheet as single-pair
    lookups (Partner='World' for the total row), so no new sheet/schema is
    needed."""
    mode_label = "bilateral_compare"
    ref = get_country_reference()
    r_code, r_name, r_iso3 = resolve_country_code(reporter, ref)
    flow_code = "X" if str(flow).strip().upper() in ("X", "EXPORT", "EXPORTS") else "M"
    ...
    log.info("\n" + "█" * 60)
    log.info(f"  GEOGRAPHY DRILL-DOWN — COMPARE PARTNERS for {r_name} ({r_code})")
    log.info(f"  Flow: {'Exports' if flow_code == 'X' else 'Imports'}  |  "
             f"Partners: {len(partners)}  |  HS codes: {len(use_hs)}  |  "
             f"Years: {use_years[0]}–{use_years[-1]}  |  Include World total: {include_world}")
    log.info("█" * 60)
    ...
    for partner in partners:
        p_code, p_name, p_iso3 = resolve_country_code(partner, ref)
        if p_code == r_code:
            log.warning(f"  ⚠  Skipping {p_name} — same as reporter.")
            continue
        log.info(f"\n  -- Partner: {p_name} ({p_code}) --")
        raw  = fetch_bilateral(config, r_code, p_code, flow_code, use_hs, use_years, mode_label=mode_label)
        rows = build_bilateral_rows(raw, r_name, r_iso3, p_name, p_iso3, config)
        if rows.empty:
            log.warning(f"     ⚠  No data for {r_name} ↔ {p_name} — skipped.")
        else:
            all_rows.append(rows)

    if include_world:
        log.info(f"\n  -- World total (for % share + Rest of World) --")
        raw_w  = fetch_reporter_total(config, r_code, flow_code, use_hs, use_years, mode_label=mode_label)
        rows_w = build_bilateral_rows(raw_w, r_name, r_iso3, "World", "", config)
        if rows_w.empty:
            log.warning("     ⚠  No World-total data returned — % share will be relative to selected partners only.")
        else:
            all_rows.append(rows_w)

    if not all_rows:
        log.warning("  ⚠  No data returned for any selected partner — nothing added to the workbook.")
        return pd.DataFrame()

    combined_new = pd.concat(all_rows, ignore_index=True, sort=False)
    write_bilateral_sheet(config, combined_new)
    log.info(f"\n  ✅  GEOGRAPHY DRILL-DOWN COMPARE COMPLETE. {len(combined_new)} new row(s) added to "
             f"Geography_Bilateral in {config['cleaned_file']}.")
    return combined_new


def fetch_top_partners(config, reporter_code, flow_code, hs_codes, years,
                        mode_label="top_partners", use_cache=True):
    """Single batched fetch of a reporter's FULL partner breakdown — every
    country it trades with for this flow/HS/year scope, not just one — used
    only to RANK partners by trade value so the user can see who is worth
    comparing. This is still one API call per (HS-batch × year-batch), same
    limits as fetch_bilateral/fetch_reporter_total; it never loops per
    country. The trick is omitting partnerCode entirely (passed as None):
    comtradeapicall.getFinalData() drops any None-valued parameter before
    building the request URL, so the Comtrade API itself returns one row
    per partner country instead of requiring a separate call per partner.
    Mirrors fetch_reporter_total's chunking/caching exactly and shares its
    on-disk cache file, under a distinct 'PARTNERS|...' key prefix so the
    three fetchers never collide."""
    import comtradeapicall
    ...
    for hs_chunk in hs_chunks:
        for yr_chunk in yr_chunks:
            key = "|".join(["PARTNERS", str(reporter_code), str(flow_code),
                            ",".join(sorted(hs_chunk)), ",".join(sorted(yr_chunk))])
            if use_cache and key in cache:
                rows = cache[key]
                if rows:
            ...
            period_str = ",".join(yr_chunk)
            cmd_str    = ",".join(hs_chunk)
            log.info(f"  → Top-partners fetch: reporter={reporter_code} flow={flow_code} "
                     f"HS={cmd_str} years={period_str} (all partners, one call)")
            try:
                mydf = comtradeapicall.getFinalData(
                    api_key, typeCode="C", freqCode="A", clCode="HS",
                    period=period_str, reporterCode=reporter_code, cmdCode=cmd_str,
                    flowCode=flow_code, partnerCode=None, partner2Code=None,
                    customsCode=None, motCode=None, maxRecords=None,
                    format_output="JSON", aggregateBy=None, breakdownMode="classic",
                    countOnly=None, includeDesc=True,
                )
                if mydf is None or mydf.empty:
                    log.warning(f"     ⚠  No partner-breakdown data returned for HS {cmd_str}, years {period_str}")
                    cache[key] = []
                    record_error(mode_label, f"{flow_code}:{reporter_code}->ALL",
                                 cmd_str, period_str, "NoData", "API returned empty/None",
                                 "Skipped — logged")
                else:
                    n_partners = mydf["partnerCode"].nunique() if "partnerCode" in mydf.columns else "?"
                    log.info(f"     ✓  {len(mydf)} rows across {n_partners} partner countr(y/ies)")
                    frames.append(mydf)
                    cache[key] = mydf.to_dict(orient="records")
            except Exception:
                emsg = traceback.format_exc().strip().split("\n")[-1]
                ...
                record_error(mode_label, f"{flow_code}:{reporter_code}->ALL",
                             ...
                             "Skipped — logged" if config["continue_on_error"] else "Halted")
                if not config["continue_on_error"]:
                    log.error("Continue-on-error is OFF. Stopping top-partners fetch.")
                    if use_cache:
                        _save_bilateral_cache(config, cache)
                    return pd.concat(frames, ignore_index=True, sort=False) if frames else pd.DataFrame()
            time.sleep(0.5)
...
def rank_top_partners(raw_df, top_n=10):
    """Aggregate a fetch_top_partners() raw frame into a ranked partner
    table: total trade value across whatever HS codes/years were fetched,
    summed per partner country, with each partner's % share of the
    reporter's real total for this flow (the World aggregate row, partner
    code '0', is excluded first so percentages reflect only actual
    countries — consistent with how the rest of Geography Drill-Down treats
    World as a separate denominator, not just another partner)."""
    cols = ["Rank", "Partner", "Partner Code", "Partner ISO3", "Trade Value (USD)", "Share %"]
    if raw_df is None or raw_df.empty:
        return pd.DataFrame(columns=cols)

    df = raw_df.copy()
    p_code = _get(df, ["partnerCode", "partnercode"]).astype(str)
    df = df[p_code != "0"]
    if df.empty:
        return pd.DataFrame(columns=cols)

    grp = pd.DataFrame({
        "Partner Code":      _get(df, ["partnerCode", "partnercode"]).astype(str),
        "Partner":           _get(df, ["partnerDesc", "partnerdesc"]).astype(str),
        "Partner ISO3":      _get(df, ["partnerISO", "partneriso"], fallback="").astype(str),
        "Trade Value (USD)": pd.to_numeric(_get(df, ["primaryValue", "primaryvalue"]), errors="coerce"),
    }).groupby(["Partner Code", "Partner", "Partner ISO3"], as_index=False)["Trade Value (USD)"].sum()

    total = grp["Trade Value (USD)"].sum()
    grp["Share %"] = (grp["Trade Value (USD)"] / total * 100).round(2) if total else 0.0
    grp = grp.sort_values("Trade Value (USD)", ascending=False).reset_index(drop=True)
    grp.insert(0, "Rank", grp.index + 1)
    return grp.head(top_n)[cols]


def run_top_partners(config, reporter, hs_codes=None, years=None, top_n=10,
                      flow="BOTH", is_test=False):
    """CLI/UI entry point for 'find top partners': for one reporter + the
    HS-code scope/year range currently active in Geography Drill-Down,
    fetch the FULL partner breakdown (one batched call-set, never a
    per-country loop) and rank the top_n partner countries by trade value
    — so the user knows WHICH partners are worth comparing instead of
    guessing blindly. By default fetches BOTH directions (top export
    destinations AND top import sources) in one run, since deciding who to
    compare against needs both regardless of which single Flow the rest of
    the page currently has selected; pass flow='X' or 'M' to fetch only
    one direction. Writes a small JSON snapshot (not the persistent
    Geography_Bilateral sheet — this is a discovery/ranking tool, not a
    saved trade-data lookup) to '{slug}_top_partners.json'."""
    mode_label = "top_partners"
    ref = get_country_reference()
    r_code, r_name, r_iso3 = resolve_country_code(reporter, ref)

    flow_norm = str(flow).strip().upper()
    if flow_norm in ("X", "EXPORT", "EXPORTS"):
        flow_codes = ["X"]
    elif flow_norm in ("M", "IMPORT", "IMPORTS"):
        flow_codes = ["M"]
    else:
        flow_codes = ["X", "M"]
    ...
    log.info("\n" + "█" * 60)
    log.info(f"  GEOGRAPHY DRILL-DOWN — FIND TOP {top_n} PARTNERS for {r_name} ({r_code})")
    log.info(f"  Directions: {', '.join('Exports' if f == 'X' else 'Imports' for f in flow_codes)}  |  "
             f"HS codes: {len(use_hs)}  |  Years: {use_years[0]}–{use_years[-1]}")
    log.info("█" * 60)

    results = {}
    for flow_code in flow_codes:
        label = "exports" if flow_code == "X" else "imports"
        log.info(f"\n  -- {label.title()} (top destinations)" if flow_code == "X"
                  else f"\n  -- {label.title()} (top sources) --")
        raw    = fetch_top_partners(config, r_code, flow_code, use_hs, use_years, mode_label=mode_label)
        ranked = rank_top_partners(raw, top_n=top_n)
        results[label] = ranked
        if ranked.empty:
            log.warning(f"     ⚠  No partner-breakdown data returned for {label}.")
        else:
            for _, row in ranked.iterrows():
                log.info(f"     {int(row['Rank']):>2}. {row['Partner']:<30} "
                         f"${row['Trade Value (USD)']:,.0f}  ({row['Share %']}%)")

    snapshot = {
        "reporter":      r_name,
        "reporter_code": r_code,
        "hs_codes":      use_hs,
        "year_start":    use_years[0],
        "year_end":      use_years[-1],
        "top_n":         top_n,
        "generated_at":  datetime.now().isoformat(timespec="seconds"),
        "exports":       results.get("exports", pd.DataFrame()).to_dict(orient="records"),
        "imports":       results.get("imports", pd.DataFrame()).to_dict(orient="records"),
    }
    snap_path = Path(f"{config['slug']}_top_partners.json")
    snap_path.write_text(json.dumps(snapshot, indent=2), encoding="utf-8")
    log.info(f"\n  ✅  TOP PARTNERS SAVED → {snap_path}")
    return results
...
def main():
    ...
    parser.add_argument("--mode",   required=True,
                        choices=["setup_check", "test", "full", "clean", "slide_ready",
                                ...
                                "visuals_data", "all", "bilateral", "bilateral_compare",
                                "top_partners", "list_countries"],
                        help="Pipeline mode to run")
    # Geography Drill-Down (--mode bilateral / bilateral_compare / top_partners) arguments
    parser.add_argument("--reporter", help="Reporter country: name, ISO3 code, or numeric Comtrade code")
    parser.add_argument("--partner",  help="Partner country: name, ISO3 code, or numeric Comtrade code (--mode bilateral)")
    parser.add_argument("--partners", help="Comma-separated partner countries to compare (--mode bilateral_compare)")
    parser.add_argument("--flow",     help="Trade flow: X/export(s) or M/import(s). With --mode top_partners, "
                                            "omit (or pass 'both') to fetch both directions in one run.")
    parser.add_argument("--hs",       help="Comma-separated HS codes (default: full sector HS list from config)")
    parser.add_argument("--years",    help="Comma-separated years (default: full year range from config)")
    parser.add_argument("--bilateral-test", action="store_true",
                        help="With --mode bilateral/bilateral_compare/top_partners: 1 HS code, latest year only (quick connectivity check)")
    parser.add_argument("--no-world-total", action="store_true",
                        help="With --mode bilateral_compare: skip the extra World-total fetch "
                             "(share will be relative to the selected partners only, not the reporter's true total)")
    parser.add_argument("--top-n", type=int, default=10,
                        help="With --mode top_partners: how many top partner countries to keep per direction (default 10)")
    parser.add_argument("--start-year", help="Override the config file's Start Year for this run only "
                                              "(applies to every mode; the config file itself is never modified)")
    parser.add_argument("--end-year",   help="Override the config file's End Year for this run only "
                                              "(applies to every mode; the config file itself is never modified)")
    ...
    # In-memory year-range override — lets a caller (e.g. the Streamlit app)
    # fetch a different year range than whatever is hardcoded in the config
    # file's "years" sheet, without ever writing back to that file. Affects
    # every mode below, since they all read start_year/end_year/years_list/
    # period/test_year off this same config dict.
    if args.start_year or args.end_year:
        _new_start = int(args.start_year) if args.start_year else config["start_year"]
        _new_end   = int(args.end_year)   if args.end_year   else config["end_year"]
        if _new_end < _new_start:
            log.error(f"  ✗  --end-year ({_new_end}) is before --start-year ({_new_start}).")
            sys.exit(1)
        config["start_year"]  = _new_start
        config["end_year"]    = _new_end
        config["years_list"]  = list(range(_new_start, _new_end + 1))
        config["period"]      = ",".join(str(y) for y in config["years_list"])
        config["test_year"]   = str(_new_end)
        log.info(f"  ·  Year range overridden for this run: {_new_start}–{_new_end} "
                 f"(config file's own Start/End Year left untouched)")
    ...
    log.info(f"  Years   : {config['start_year']}–{config['end_year']}")
    ...
    elif args.mode == "list_countries":
        ref = get_country_reference()
        log.info(f"\n  {len(ref)} countries/areas available for Geography Drill-Down "
                 f"(reporter and partner share the same list):")
        for _, r in ref.iterrows():
            log.info(f"    {r['code']:<6} {r['iso3']:<5} {r['name']}")
        log.info(f"\n  Use any of: numeric code, ISO3 code, or country name with --reporter/--partner.\n")

    elif args.mode == "bilateral":
        if not args.reporter or not args.partner or not args.flow:
            log.error("  ✗  --mode bilateral requires --reporter, --partner, and --flow.")
            log.error("     Example: --mode bilateral --reporter China --partner \"United States\" --flow import")
            sys.exit(1)
        hs_codes = [c.strip() for c in args.hs.split(",")] if args.hs else None
        years    = [y.strip() for y in args.years.split(",")] if args.years else None
        try:
            run_bilateral(config, args.reporter, args.partner, args.flow,
                          hs_codes=hs_codes, years=years, is_test=args.bilateral_test)
            log.info(
                f"\n  GEOGRAPHY DRILL-DOWN COMPLETE. Open {config['cleaned_file']} → "
                f"Geography_Bilateral sheet.\n"
            )
        except ValueError as e:
            log.error(f"  ✗  {e}")
            sys.exit(1)

    elif args.mode == "bilateral_compare":
        if not args.reporter or not args.partners or not args.flow:
            log.error("  ✗  --mode bilateral_compare requires --reporter, --partners, and --flow.")
            log.error("     Example: --mode bilateral_compare --reporter China "
                      "--partners \"Australia,Brazil,Argentina\" --flow import")
            sys.exit(1)
        hs_codes = [c.strip() for c in args.hs.split(",")] if args.hs else None
        years    = [y.strip() for y in args.years.split(",")] if args.years else None
        partners = [p.strip() for p in args.partners.split(",") if p.strip()]
        try:
            run_bilateral_compare(config, args.reporter, partners, args.flow,
                                  hs_codes=hs_codes, years=years,
                                  include_world=not args.no_world_total,
                                  is_test=args.bilateral_test)
            log.info(
                f"\n  GEOGRAPHY DRILL-DOWN COMPARE COMPLETE. Open {config['cleaned_file']} → "
                f"Geography_Bilateral sheet.\n"
            )
        except ValueError as e:
            log.error(f"  ✗  {e}")
    ...
    elif args.mode == "top_partners":
        if not args.reporter:
            log.error("  ✗  --mode top_partners requires --reporter.")
            log.error("     Example: --mode top_partners --reporter India --top-n 10")
            sys.exit(1)
        hs_codes = [c.strip() for c in args.hs.split(",")] if args.hs else None
        years    = [y.strip() for y in args.years.split(",")] if args.years else None
        try:
            run_top_partners(config, args.reporter, hs_codes=hs_codes, years=years,
                             top_n=args.top_n, flow=(args.flow or "BOTH"),
                             is_test=args.bilateral_test)
            log.info(
                f"\n  TOP PARTNERS COMPLETE. See {config['slug']}_top_partners.json for the ranked list.\n"
            )
        except ValueError as e:
            log.error(f"  ✗  {e}")
            sys.exit(1)
