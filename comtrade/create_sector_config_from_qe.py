...
def build_config(
    ...
    default_flows = [
        # Output File names are prefixed with this sector's own slug (not a
        # generic "out_*.xlsx") so two sectors' raw pulls never collide or
        # silently overwrite each other when both configs live in the same
        # working folder.
        ("World Exports", "X", "All (None)", "None", "World", "0", f"out_{slug}_world_exports.xlsx"),
        ("World Imports", "M", "All (None)", "None", "World", "0", f"out_{slug}_world_imports.xlsx"),
        ("India Exports", "X", "India",      "699",  "World", "0", f"out_{slug}_india_exports.xlsx"),
        ("India Imports", "M", "India",      "699",  "World", "0", f"out_{slug}_india_imports.xlsx"),
    ]
